...
def fetch_data1(inp_surename, db_type, output):
    data_birth_total, data_death_total, data_wed_total = [[]], [[]], [[]], [[]], [[]], [[]]
    data_death = []
    #db_type_wed = "HUNGMARR26" # wedding
    db_type_birth = "HUNGBIRTH27" # birth
    db_type_death = "HUNGDEATH26" # death
    ...
    x = 1
    # get first page
    data_death = get_data(db_type_death, i, surename)
    data_death_total = data_death

    print("First Fetch:", len(data_death), len(data_death_total))
    if len(data_death) == 2:
        ...
        while x == 1:
            ...
            print("Next Fetch:", i, len(data_death), len(data_death_total))
            if len(data_death) == 2:
                x = 0
        print("After Loop:", i, len(data_death), len(data_death_total))
        build_jg_death(data_death_total, data_death_out)
        print("Build:", len(data_death_out), data_death_out)
    ...
    print("After loop:", i, len(data_death), len(data_death_total), len(data_death_out))
    ...
    data_birth = get_data(db_type_birth, i, surename)
